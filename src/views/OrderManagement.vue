...
<script>
import axios from '../assets/axios/OrderManagement'
...
export default {
    ...
    setup() {
        let { page, size, total, userList, handleCurrentChange, getOrderList } = getTableData()
        onMounted(() => {
            getOrderList()
        })
...
function getTableData() {
    ...
    function getOrderList() {
        axios.getOrderList((page.value - 1) * size, size)
        .then(({data}) => {
            if(data.status === 0) {
